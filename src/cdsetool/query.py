...
def _validate_search_terms(collection, search_terms):
    ...
    for key in search_terms.keys():
        assert (
            key in keys
        ), f"search_term with name \"{key}\" was not found for collection \"{collection}\". Available terms are: {', '.join(keys)}"
...
def _get_describe_doc(collection):
    ...
    res = requests.get(
        f"https://catalogue.dataspace.copernicus.eu/resto/api/collections/{collection}/describe.xml"
    )
    assert (
        res.status_code == 200
    ), f"Unable to find collection with name {collection}. Please see https://documentation.dataspace.copernicus.eu/APIs/OpenSearch.html#collections for a list of available collections"
